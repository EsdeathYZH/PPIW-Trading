...
bool
ubi_thread::register_completion_callback(callback_t callback)
{
  completion_callbacks().push_back(callback);
  return true;
}

vector<ubi_thread::callback_t> &
ubi_thread::completion_callbacks()
{
...
void *
ubi_thread::pthread_bootstrap(void *p)
{
  ubi_thread *self = static_cast<ubi_thread *>(p);
  try {
    self->run();
  } catch (...) {
